...
def test_fill_defaults(products):
    df = pd.DataFrame(**products)
    df, md_dict = rf.fill_defaults(
        df, dict(attr1='plastic', attr5='taxable'))
    pd.testing.assert_series_equal(
        df['attr1'], pd.Series(
            ['plastic', 'copper', 'steel', 'aluminum'], name='attr1'))
    pd.testing.assert_series_equal(
        df['attr5'], pd.Series(
            ['taxable', 'taxable', 'taxable', 'taxable'], name='attr5'))
    expected_metadata = pd.DataFrame([
        dict(id=0, name=0, price=0, cost=0, upc=0, attr1=1, attr2=0,
             attr3=0, attr4=0, attr5=4)
    ])
    pd.testing.assert_frame_equal(md_dict['metadata'], expected_metadata)
