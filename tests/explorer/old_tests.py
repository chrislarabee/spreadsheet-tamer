...
def test_count_values(employees):
    ...
    expected = pd.DataFrame([dict(employee_id=4, department=4, name=4, wfh_stipend=2)])
...
def test_count_uniques(customers, sales, products):
    ...
    expected = pd.DataFrame([dict(id=4, fname=4, lname=4, foreign_key=4)])
    df, md_dict = ex.count_uniques(pd.DataFrame(**customers()))
    pd.testing.assert_frame_equal(md_dict["metadata"], expected)

    expected = pd.DataFrame([dict(location=4, region=2, sales=4)])
...
def test_id_type_violations():
    ...
    df, md_dict = ex.id_type_violations(df, dict(a=int, b=float, c=str))
    pd.testing.assert_frame_equal(md_dict["metadata"], expected)
...
def test_id_clustering_violations():
    ...
    df, md_dict = ex.id_clustering_violations(df, ["a", "b"], ["c"])
    pd.testing.assert_frame_equal(df[expected_cols], expected)
    expected_metadata = pd.DataFrame([dict(a=0, b=0, c=3)])
    ...
    df, md_dict = ex.id_clustering_violations(df, ["a"], [("b", "c")])
    pd.testing.assert_frame_equal(df[expected_cols], expected)
    expected_metadata = pd.DataFrame([dict(a=0, b=2, c=3, b_c_x=3)])
    pd.testing.assert_frame_equal(md_dict["metadata"], expected_metadata)
